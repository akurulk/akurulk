		...
		function AkuruHandler(callback, options) {
			...
			this.writeText = function DefaultHandler$writeText (element) {
				...
				if(tagSpec.convert) {
					...
					results += "<span class='akuru-converted' style='" + fontFamily + "'>" + sinhalaConverter.convert(element.raw, fontname) + "</span>";
				} else {
					results += element.raw;
				}
			} 
			...
			function createAttrs(raw) {
				...
				for(var lc=1; lc<parts.length; lc++) {
					...
					attr.escaped = attr.value = (innerParts[1])? innerParts[1].trim().replace(/'/g, "").replace(/"/g, ""): "";
