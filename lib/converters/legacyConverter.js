	...
	function LegacyConverter() {
		...
		this.convert = function(unicodeText, fontname) {
			...
			tokenizer.onVowel(function(char) {
				...
				flushOtherBuffer();
				output += (texts[char])? texts[char] : getAdditionalChars(char, fontname);
			});
			...
			tokenizer.onMixed(function(part1, part2) {
				...
				if(texts[mixed]) {
					output += texts[mixed];
				} else if(getAdditionalChars(mixed, fontname)) { 
					output += getAdditionalChars(mixed, fontname);
				} else {
					output += texts[part1];
					output += texts[part2];
				}
		...
		function getAdditionalChars(char, fontname) {
			...
			var type = fontname.substring(0, 2);
			return (types[type])? types[type][char]: null;
		}
